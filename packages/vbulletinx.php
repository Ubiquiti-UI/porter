...
$Supported['vbulletinx'] = array('name'=>'vBulletin 3 & 4', 'prefix'=>'vb_');
// Commented commands are still supported, if you really want to use them.
$Supported['vbulletinx']['CommandLine'] = array(
...
$Supported['vbulletinx']['features'] = array(
   'Comments'        => 1,
   'Discussions'     => 1,
   'Users'           => 1,
   'Categories'      => 1,
 ...
 * @package VanillaPorter
 */
class Vbulletinx extends ExportController {
   ...
   public $AvatarSelect = "case
      when a.userid is not null then concat('customavatars/', uin.userid % 100,'/avatar_', uin.userid, right(a.filename, instr(reverse(a.filename), '.')))
...
//          16 => 'Plugins.WhosOnline.ViewHidden')
   ...
   protected function ForumExport($Ex) {
      ...
      // Users
      $Ex->Query("drop table if exists z_userid_new");
      $Ex->Query("create table z_userid_new (
        userid int(11) unsigned not null AUTO_INCREMENT,
        olduserid bigint(20) not null,
        primary key (userid),
        key idx_olduserid (olduserid)
      )");
      $Ex->Query("insert into z_userid_new (olduserid) select userid from :_user");
      ...
      $Ex->ExportTable('User', "select u.*,
            uin.userid as newuserid,
            rin.userid as newreferrerid,
            ...
            case when avatarrevision > 0 then concat('$cdn', 'userpics/avatar', uin.userid, '_', avatarrevision, '.gif')
                 when av.avatarpath is not null then av.avatarpath
                 else null
                 end as filephoto,
            {$this->AvatarSelect},
         ...
         from :_user u
         left join z_userid_new uin
            on u.userid = uin.olduserid
         left join z_userid_new rin
            on u.referrerid = rin.olduserid
      ...
      $Ex->Query("insert into VbulletinRoles (userid, usergroupid)
         select uin.userid, u.usergroupid
         from :_user u left join z_userid_new uin on u.userid = uin.olduserid");
      # Put stupid CSV column into tmp table
      $SecondaryRoles = $Ex->Query("select uin.userid, u.usergroupid, u.membergroupids
        from :_user u left join z_userid_new uin on u.userid = uin.olduserid", TRUE);
...
//      return;
      ...
      foreach($UserFields as $Field => $InsertAs)
         $Ex->Query("insert into VbulletinUserMeta (UserID, Name, Value)
          select uin.userid, 'Profile.$InsertAs', $Field
          from :_user u left join z_userid_new uin on u.userid = uin.olduserid
          where $Field != ''");
      ...
      if (is_resource($ProfileFields)) {
         ...
         while ($Field = @mysql_fetch_assoc($ProfileFields)) {
            ...
            $ProfileQueries[] = "insert into VbulletinUserMeta (UserID, Name, Value)
               select uin.userid, 'Profile.".$Name."', ".$Column."
               from :_userfield uf left join z_userid_new uin on uf.userid = uin.olduserid
            where ".$Column." != ''";
         }
      ...
      $Sql = "
         select
            uin.userid as UserID,
            'Plugin.Signatures.Sig' as Name,
            signature as Value
         from :_usertextfield utf left join z_userid_new uin on utf.userid = uin.olduserid
      ...
      $Discussion_Map = array(
         ...
         'forumid' => 'CategoryID',
         'newpostuserid' => 'InsertUserID',
         'newpostuserid2' => 'UpdateUserID',
      ...
      $Ex->ExportTable('Discussion', "select t.*,
            uin.userid as newpostuserid,
            uin.userid as newpostuserid2,
            p.ipaddress,
            p.pagetext as Body,
            'BBCode' as Format,
         ...
         from :_thread t
            left join z_userid_new uin on t.postuserid = uin.olduserid
      ...
      $Ex->ExportTable('Comment', "select p.*,
            'BBCode' as Format,
            uin.userid as InsertUserID,
            uin.userid as UpdateUserID,
         FROM_UNIXTIME(p.dateline) as DateInserted,
            FROM_UNIXTIME(p.dateline) as DateUpdated
         from :_post p
         left join z_userid_new uin on p.userid = uin.olduserid
      ...
      $Ex->ExportTable('UserDiscussion', "select
         ...
         left join z_userid_new uin on st.userid = uin.olduserid
      ...
      if ($Ex->Exists('visitormessage')) {
         ...
         $Ex->ExportTable('Activity', "select *,
               ...
               uin.userid as ActivityUserID,
               puin.userid as InsertUserID,
               puin.userid as RegardingUserID,
               -1 as NotifyUserID,
               'BBCode' as Format,
               'WallPost' as ActivityType
            from :_visitormessage vm
            left join z_userid_new uin on vm.userid = uin.olduserid
            left join z_userid_new puin on vm.postuserid = puin.olduserid
            where state='visible'
   ...
   protected function _ExportConversations($MinDate) {
      ...
      select
        pmtextid,
        uin.userid
      from :_pm pm left join z_userid_new uin on pm.userid = uin.olduserid
      $MinWhere");
      ...
      select
        pmtextid,
        uin.userid
      from :_pmtext pmt left join z_userid_new uin on pmt.fromuserid = uin.olduserid
      $MinWhere;");
      ...
      left join z_userid_new uin on r.userid = uin.olduserid
      $MinWhere;");
      ...
      from :_pm pm
      join :_pmreceipt r on pm.pmid = r.pmid
      left join z_userid_new uin on r.touserid = uin.olduserid
      $MinWhere;");
      ...
      $Conversation_Map = array(
         ...
         'newfromuserid' => 'InsertUserID',
         'title2' => array('Column' => 'Subject', 'Type' => 'varchar(250)')
      );
      $Ex->ExportTable('Conversation',
      'select
         pm.*,
         uin.userid as newfromuserid,
         g.title as title2,
         FROM_UNIXTIME(pm.dateline) as DateInserted
       from :_pmtext pm
       left join z_userid_new uin on pm.fromuserid = uin.olduserid
       join z_pmgroup g on g.group_id = pm.pmtextid', $Conversation_Map);
   ...
   public function ExportMedia($MinDiscussionID = FALSE) {
      ...
      if ($Ex->Exists('attachment', array('contenttypeid', 'contentid')) === TRUE) {
         ...
         $Extension = ExportModel::FileExtension('a.filename');
         $MediaSql = "select a.attachmentid, a.filename, $Extension as extension $AttachColumnsString, a.userid, uin.userid as newuserid,
               'local' as StorageMethod,
            ...
            from :_thread t
               ...
               left join z_userid_new uin on a.userid = uin.olduserid
            where a.attachmentid > 0

            union all

            select a.attachmentid, a.filename, $Extension as extension $AttachColumnsString, a.userid, uin.userid as newuserid,
               'local' as StorageMethod,
...
//      $fp = fopen('php://output', 'ab');
      ...
      $Ex->ExportTable('Poll',
         "select
            p.*,
            t.threadid,
            uin.userid as postuserid,
            !p.public as anonymous
         from :_poll p
         join :_thread t on p.pollid = t.pollid
         left join z_userid_new uin on t.postuserid = uin.olduserid", $Poll_Map);
      ...
      $Ex->ExportTable('PollVote',
         "select pv.*, uin.userid as newuserid, pollid * 1000 + voteoption as optionid
         from :_pollvote pv
         left join z_userid_new uin on pv.userid = uin.olduserid", $PollVote_Map);
   }
   ...
   function BuildMediaPath($Value, $Field, $Row) {
      if (isset($Row['hash']) && $Row['hash'] != '') {
         ...
         $n = strlen($Row['newuserid']);
         $DirParts = array();
         for($i = 0; $i < $n; $i++) {
            $DirParts[] = $Row['newuserid']{$i};
         }
